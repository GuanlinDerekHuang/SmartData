...
TRACK_CSV = first(["target_track.csv"])
...
print(f"track : {TRACK_CSV if TRACK_CSV else '(none - target marker will be static)'}")
...
def load_track_csv(path):
    """frame_idx -> list of (label, cx, cy) in processed-mask coords."""
    by_frame = {}
    with open(path) as f:
        next(f, None)
        for line in f:
            ...
            if len(parts) < 4 or parts[2] == "":
                continue
            fi, label, cx, cy = int(parts[0]), parts[1], float(parts[2]), float(parts[3])
            by_frame.setdefault(fi, []).append((label, cx, cy))
    return by_frame
...
def main():
    ...
    track = load_track_csv(TRACK_CSV) if TRACK_CSV else None
    ...
    # Frame-0 prediction: only shown statically when we have no per-frame track.
    if molmo and not track:
        pts, labels = [], []
    ...
    for i, (ts_ms, quat, trans) in enumerate(rows):
        ...
        # ---- tracked target object (moves with the camera) ----
        if track is not None:
            recs = track.get(min(i, max(track) if track else 0), [])
            if recs:
                rr.log(RGB_ENTITY + "/target",
                       rr.Points2D([[cx * sx, cy * sy] for _, cx, cy in recs],
                                   labels=[lab for lab, _, _ in recs],
                                   radii=9.0,
                                   colors=[[255, 200, 0]] * len(recs)))
